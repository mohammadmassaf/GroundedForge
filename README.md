...
`demo_corpus/` is **not** mine to license. It holds RFC 768, 791 and 793,
IETF documents whose copyright rests with the IETF Trust and which are
redistributed here under their own terms. The MIT licence above applies to
this repository's code, not to the RFC text or to any material you point it at.

Nothing in `data/`, `chunks/job.json`, `chunks/networks.json` or `chroma_db/`
is in this repository, deliberately: those are course PDFs under copyright and
personal notes, and the reason the public demo runs on RFCs instead is that
they are the part a stranger is allowed to have.
