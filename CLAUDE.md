...
- **Big picture before I build.** Before I start a function/feature, walk the end-to-end flow with a concrete example (real data shapes, who produces what, who consumes my output, why it exists). I don't want to work blindly on mechanics.
- **List new tools explicitly.** When a TODO needs library functions/methods I haven't used before, list them up front: name, what it takes, what it returns, why it's needed here. Don't bury them in prose.
- **Gloss unfamiliar words inline — don't wait for me to ask.** English isn't my first language. This covers two kinds of word: *technical jargon* (idempotent, bind mount, layer cache) and *ordinary English used figuratively* (a "seam" in the code, holding something "hostage", the "blast radius" of a bug). Both stop me the same way. Put a short gloss right where the word is — a few words in brackets, e.g. "the seam (the single place where two parts of the code meet, so a change only has to happen once)". Keep it inline; don't break into a paragraph or a glossary at the end. If a term is central to the concept, define it properly once and then use it freely.
